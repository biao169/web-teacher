...
def main():
    ...
    source=args.launchers/'windows-oneclick'
    for path in source.rglob('*'):
        if path.is_file():inventory.append((path,'test-examples/support/windows-oneclick/'+path.relative_to(source).as_posix()))
    for source in [args.additions]:
        for path in source.rglob('*'):
