...
const hasDetails = computed(() => Boolean(
  profile.value?.contact?.email || profile.value?.contact?.phone || profile.value?.contact?.office || profile.value?.links?.length,
))
</script>
<template><section id="profile" class="public-home-profile" aria-labelledby="public-home-title"><div class="public-container">
  <div v-if="profile" class="public-home-profile__layout" :class="{ 'public-home-profile__layout--single': !hasDetails }">
    <div class="public-home-profile__body">
      <div class="public-home-profile__intro">
      ...
      <p v-if="profile.organization || profile.lab" class="public-home-profile__organization">{{ [profile.organization, profile.lab].filter(Boolean).join(' · ') }}</p>
      ...
      <p v-if="profile.role || profile.title" class="public-home-profile__meta">{{ [profile.role, profile.title].filter(Boolean).join(' · ') }}</p>
        </div>
      </div>
      <p v-if="profile.biography" class="public-home-profile__biography">{{ profile.biography }}</p>
    </div>
    <aside v-if="hasDetails" class="public-home-profile__details" aria-labelledby="public-home-details-title">
      <h2 id="public-home-details-title">{{ model.locale === 'zh' ? '个人资料' : 'Profile' }}</h2>
