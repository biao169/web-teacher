...
const report = {
  stage: 5,
  scope: 'pinned production dependencies, native SQLite, workerd, Nuxt builds, browser SSR/hydration, accessibility and public performance release gates',
  generatedAt: new Date().toISOString(),
  environment: { node: process.version, platform: process.platform, arch: process.arch },
  status: 'running', blockers: [], checks: [],
}
...
for (const name of ['typescript','vitest','nuxt','vue','h3','zod','nuxt-security','drizzle-orm','better-sqlite3','@cloudflare/vitest-plugin','@cloudflare/workers-types','tsx','wrangler','tailwindcss','@tailwindcss/vite','@lucide/vue','@playwright/test']) {
...
if (report.blockers.length) {
  report.status = 'blocked_prerequisites'; process.exitCode = 2
}
else {
  for (const name of ['verify:stage5:offline','typecheck:stage5','typecheck:database','test:database:native','test:database:workerd','typecheck','lint','test:unit','build:ubuntu','smoke:ubuntu','build:cloudflare','smoke:cloudflare','test:e2e']) {
    const result = spawnSync(manager, ['run', name], { cwd: root, encoding: 'utf8', timeout: 600_000, maxBuffer: 64 * 1024 * 1024, env: { ...process.env, NO_COLOR: '1' } })
