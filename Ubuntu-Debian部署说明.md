...
- Ubuntu/Debian 目标使用生产瘦身依赖，不安装 Cloudflare CLI、Workers 本地模拟和测试工具；小内存机器若有 `swapon/mkswap` 会临时启用 `/opt/academic-suite/build.swap`，没有则跳过。备份需**完整现有目录空间＋至少1GiB余量**。
...
| `source` | 拉取源码、安装 Ubuntu/Debian 生产依赖、检查待迁移、构建 | 有待迁移则停止；启用快传时拒绝此模式，请用 full |
...
三个密钥分别随机生成。服务默认以系统已有 `nobody:nogroup` 非 root 账号执行，设置私有临时目录、禁止提权、日志交给 journal。依赖安装和应用构建在部署期由当前 root 管理命令执行，运行期才切换到低权限服务账号；请只部署自己审核过的仓库。
