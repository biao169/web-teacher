...
export function runProcess(command, args, options = {}) {
  ...
  return new Promise(resolve => {
    ...
    let timer
    let settled = false
    const child = spawn(command, args, { cwd: options.cwd, env: { ...process.env, ...options.env }, shell: false, detached: process.platform !== 'win32', stdio: options.inherit ? 'inherit' : ['ignore', 'pipe', 'pipe'] })
    const kill = signal => {
      if (!child.pid) return
