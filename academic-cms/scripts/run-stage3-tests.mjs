...
const output = await mkdtemp(resolve(tempRoot, 'stage3-core-'))
...
try {
  ...
  const compile = spawnSync(compiler.command, [
    ...compiler.prefix, '-p', 'tsconfig.stage3.json', '--noEmit', 'false',
  ...
  if (compile.status !== 0 || compile.signal) {
    if (compile.signal) throw new Error(`Stage-3 compilation terminated by ${compile.signal}`)
    exitCode = compile.status ?? 1
  }
  else {
    const files = (await readdir(resolve(root, 'tests/stage3'), { withFileTypes: true }))
      .filter(item => item.isFile() && item.name.endsWith('.spec.mjs'))
      .map(item => `tests/stage3/${item.name}`).sort()
    if (!files.length) throw new Error('No stage-3 tests found')
    const result = spawnSync(process.execPath, [
      ...
      stdio: 'inherit',
      timeout: 120_000,
      env: { ...process.env, NO_COLOR: '1', STAGE3_CORE_OUTPUT: output },
    })
    if (result.error) throw result.error
    if (result.signal) throw new Error(`Stage-3 tests terminated by ${result.signal}`)
